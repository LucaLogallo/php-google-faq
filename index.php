...
<body>
  ...
  <header>
    <div class="row sopra">
    <!-- logo google con link text -->
      <div class="left">
        <img src="assets/img/Google-logo.png" alt="">
        <a href="#">Privacy e termini</a>
      </div>
    <!-- favicon quadrato puntini e tasto accedi -->
      <div class="right">
        <button><i class="fas fa-th"></i></button>
        <button>ACCEDI</button>
      </div>
    </div>
    <!-- row che contiene una lista  -->
    <div class="row sotto">
      <ul>
        <?php for($i=0; $i<count($listaMenu); $i++){
          ?>
        <li><?php echo $listaMenu[$i];?></li>
        <?php } ?>
      </ul>
    </div>
  </header>
    ...
    <div class="container">
      <ul>
        <?php foreach($data as $dato){
          $explode = explode(PHP_EOL, $dato['answer']);
          ?>
        <li class="title"><?php echo $dato['question'] ?></li>
        <?php foreach($explode as $p){?>
        <li class="text"><?php echo $p ?></li>
